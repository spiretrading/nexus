...
TEST_SUITE("MoldUdp64Message") {
  TEST_CASE("parse_valid_message") {
    ...
    auto length_be = ToBigEndian(length);
    ...
    std::memcpy(buffer.data() + sizeof(std::uint16_t) + sizeof(std::uint8_t),
      data, data_length);
    auto message = MoldUdp64Message::parse(buffer.data(), buffer.size());
  ...
  TEST_CASE("parse_too_short_header") {
    ...
    REQUIRE_THROWS_AS(MoldUdp64Message::parse(buffer.data(), buffer.size()),

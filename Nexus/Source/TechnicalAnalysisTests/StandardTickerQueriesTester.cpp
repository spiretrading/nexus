...
TEST_SUITE("StandardTickerQueries") {
  TEST_CASE("load_open") {
    ...
    auto result = std::async(std::launch::async, [&] {
      return load_open(client, ticker, date, get_default_time_zone_database());
    });
  ...
  TEST_CASE("make_daily_query_range") {
    ...
    auto range = make_daily_query_range(
      ticker, start_date, end_date, get_default_time_zone_database());
  ...
  TEST_CASE("make_query") {
    ...
    SUBCASE("time_and_sale") {
      ...
      auto query = make_query(
        ticker, start_date, end_date, get_default_time_zone_database(), low);

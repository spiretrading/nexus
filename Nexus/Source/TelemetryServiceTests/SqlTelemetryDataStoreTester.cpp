...
namespace {
  using TestSqlTelemetryDataStore =
    SqlTelemetryDataStore<SqlConnection<Connection>>;
}

TEST_SUITE("LocalTelemetryDataStore") {
  TEST_CASE("load_empty_events") {
    auto dataStore = TestSqlTelemetryDataStore([=] {
      return SqlConnection<Connection>("file:db?mode=memory&cache=shared");
    });

...
TEST_SUITE("TickerEntry") {
  ...
  TEST_CASE("publish_time_and_sale") {
    ...
    auto time_and_sale2 = TimeAndSale(
      time_from_string("2024-07-11 14:00:01"), Money::ONE + Money::CENT, 200,
      TimeAndSale::Condition(), "TSX", "", "");
  ...
  TEST_CASE("snapshot") {
    ...
    auto book_bid1 = BookQuote("MP1", false, TSX,
      make_bid(10 * Money::CENT, 100), time_from_string("2024-07-11 13:00:00"));
    expected_snapshot.m_bids.push_back(entry.publish(book_bid1, 1).get());
    auto book_ask1 = BookQuote("MP1", false, TSX,
      make_ask(11 * Money::CENT, 200), time_from_string("2024-07-11 13:00:00"));
    ...
    auto bbo2 = BboQuote(make_bid(12 * Money::CENT, 300),
      make_ask(13 * Money::CENT, 400), time_from_string("2024-07-11 13:00:01"));
    expected_snapshot.m_bbo_quote = entry.publish(bbo2, 1).get();
    REQUIRE((entry.load_snapshot() == expected_snapshot));
    REQUIRE(**entry.get_bbo_quote() == expected_snapshot.m_bbo_quote);
    auto book_bid2 = BookQuote("MP2", false, TSX,
      make_bid(9 * Money::CENT, 500), time_from_string("2024-07-11 13:00:02"));
    ...
    auto book_bid1_update = BookQuote("MP1", false, TSX,
      make_bid(10 * Money::CENT, 700), time_from_string("2024-07-11 13:00:03"));
  ...
  TEST_CASE("technicals") {
    ...
    auto entry = TickerEntry(ticker, DEFAULT_VENUES,
      get_default_time_zone_database(), Money::ONE, initial_sequences);
    auto technicals1 = entry.get_ticker_technicals();
    ...
    auto ts1 = TimeAndSale(time_from_string("2024-07-11 09:30:01"),
      ...
      "");
    entry.publish(ts1, 1);
    auto technicals2 = entry.get_ticker_technicals();
    ...
    auto ts2 = TimeAndSale(time_from_string("2024-07-11 09:30:02"),
      ...
      "");
    entry.publish(ts2, 1);
    auto technicals3 = entry.get_ticker_technicals();
    ...
    auto ts3 = TimeAndSale(time_from_string("2024-07-11 09:30:03"),
      ...
      "");
    entry.publish(ts3, 1);
    auto technicals4 = entry.get_ticker_technicals();

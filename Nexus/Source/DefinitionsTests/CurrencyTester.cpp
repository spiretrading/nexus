...
TEST_SUITE("Currency") {
  ...
  TEST_CASE("shuttle") {
    test_round_trip_shuttle(CURRENCIES, [] (const auto& currencies) {
  ...
  TEST_CASE("parse_currency") {
    auto database = CurrencyDatabase();
    database.add({CurrencyId(840), "USD", "US Dollar", "$"});
  ...
  TEST_CASE("parse_currency_database_entry") {
    ...
    REQUIRE(entry.m_name == "US Dollar");

...
TEST_SUITE("TimeAndSale") {
  TEST_CASE("stream_condition") {
    ...
    REQUIRE(to_string(TimeAndSale::Condition::Type::REOPEN) == "REOPEN");
    REQUIRE(to_string(TimeAndSale::Condition::Type::AUCTION) == "AUCTION");
    REQUIRE(
      to_string(TimeAndSale::Condition::Type::CORRECTION) == "CORRECTION");
    REQUIRE(
      to_string(TimeAndSale::Condition::Type::CANCELLATION) == "CANCELLATION");
  }
  ...
  TEST_CASE("stream") {
    auto time_and_sale = TimeAndSale(
      ...
      TimeAndSale::Condition(TimeAndSale::Condition::Type::CANCELLATION, "X"),
      "NYSE", "BUY123", "SELL456");
    auto expected = std::stringstream();
    expected << '(' << time_and_sale.m_timestamp << ' ' <<
      time_and_sale.m_price << ' ' << time_and_sale.m_size << ' ' <<
      time_and_sale.m_condition << ' ' << time_and_sale.m_market_center <<
      ' ' << time_and_sale.m_buyer_mpid << ' ' << time_and_sale.m_seller_mpid <<
      ')';

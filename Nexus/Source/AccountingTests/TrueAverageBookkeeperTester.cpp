...
TEST_SUITE("TrueAverageBookkeeper") {
  TEST_CASE("empty_bookkeeper") {
    ...
    REQUIRE(is_empty(usd_inventory));
    auto cad_total = bookkeeper.get_total(CAD);
    REQUIRE(is_empty(cad_total));
    auto usd_total = bookkeeper.get_total(USD);
    REQUIRE(is_empty(usd_total));
  }
  ...
  TEST_CASE("multiple_inventories_and_currencies") {
    ...
    auto& msft_inventory = bookkeeper.get_inventory(MSFT, USD);
    ...
    auto& usd_total = bookkeeper.get_total(USD);
    REQUIRE(usd_total.m_position.m_quantity == 50);
    REQUIRE(usd_total.m_position.m_cost_basis == 7500 * Money::ONE);
    REQUIRE(usd_total.m_fees == Money::CENT);
  }
}

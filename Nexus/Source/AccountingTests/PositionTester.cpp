...
namespace {
  auto TST = parse_ticker("SHOP.TSX");
}
...
TEST_SUITE("Position") {
  ...
  TEST_CASE("get_average_price") {
    auto position1 =
      Position(TST, DefaultCurrencies::CAD, 100, 1234 * Money::ONE);
    REQUIRE(get_average_price(position1) == parse_money("12.34"));
    auto position2 =
      Position(TST, DefaultCurrencies::CAD, -100, -1234 * Money::ONE);
    REQUIRE(get_average_price(position2) == parse_money("12.34"));
    auto position3 = Position(TST, DefaultCurrencies::CAD, 0, Money::ZERO);

...
TEST_SUITE("DataTypes") {
  TEST_CASE("parse_left_padded_alpha_numeric_basic") {
    auto field = "   ABC";
    auto cursor = field;
    auto result = parse_left_padded_alpha_numeric(6, Store(cursor));
    REQUIRE(result == "ABC");
  ...
  TEST_CASE("parse_left_padded_numeric_int") {
    auto field = "   123";
    auto cursor = field;
    auto result = parse_left_padded_numeric<int>(6, Store(cursor));
    REQUIRE(result == 123);
    REQUIRE(cursor == field + 6);
  }
  ...
  TEST_CASE("parse_left_padded_numeric_all_spaces") {
    ...
    auto cursor = field;
    REQUIRE_THROWS_AS(
      parse_left_padded_numeric<int>(6, Store(cursor)), bad_lexical_cast);
    REQUIRE(cursor == field + 6);
  }

...
void TsxFeeHandlingTester::TestFeeTableCalculations() {
  ...
  TestFeeTableIndex(feeTable, feeTable.m_auctionFeeTable,
    LookupAuctionFee, TsxFeeTable::AUCTION_INDEX_COUNT,
    TsxFeeTable::AUCTION_TYPE_COUNT);
}

void TsxFeeHandlingTester::TestZeroQuantity() {
  auto feeTable = BuildFeeTable();
...
void TsxFeeHandlingTester::TestActiveInterlistedSubDollar() {
  auto feeTable = BuildFeeTable();
...
void TsxFeeHandlingTester::TestPassiveInterlistedSubDollar() {
  auto feeTable = BuildFeeTable();
  auto expectedFee = LookupContinuousFee(feeTable,

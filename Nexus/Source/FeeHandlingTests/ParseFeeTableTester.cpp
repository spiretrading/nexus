...
TEST_SUITE("ParseFeeTable") {
  TEST_CASE("unordered_map_basic") {
    ...
    parse_fee_table<Money>(yaml, Store(table));
  ...
  TEST_CASE("unordered_map_named_node") {
    auto yaml = YAML::Load(
      ...
      "  Y: 20.00\n");
    auto table = std::unordered_map<std::string, Money>();
    parse_fee_table<Money>(yaml, "fees", Store(table));
  ...
  TEST_CASE("unordered_map_missing_node") {
    ...
    auto table = std::unordered_map<std::string, Money>();
    REQUIRE_THROWS_AS(
      parse_fee_table<Money>(yaml, "fees", Store(table)), std::runtime_error);
  }

  TEST_CASE("array_basic") {
    auto yaml = YAML::Load("[1.1, 2.2, 3.3]");
    auto table = std::array<Money, 3>();
    parse_fee_table<Money, 3>(yaml, Store(table));
  ...
  TEST_CASE("array_named_node") {
    ...
    auto table = std::array<Money, 3>();
    parse_fee_table<Money, 3>(yaml, "fees", Store(table));
  ...
  TEST_CASE("array_invalid_size") {
    ...
    auto table = std::array<Money, 3>();
    REQUIRE_THROWS_AS(
      (parse_fee_table<Money, 3>(yaml, Store(table))), std::runtime_error);
  }

  TEST_CASE("2d_array_basic") {
    auto yaml = YAML::Load("[[1, 2], [3, 4]]");
    auto table = std::array<std::array<Quantity, 2>, 2>();
    parse_fee_table<Quantity, 2, 2>(yaml, Store(table));
  ...
  TEST_CASE("2d_array_named_node") {
    ...
    auto table = std::array<std::array<Quantity, 2>, 2>();
    parse_fee_table<Quantity, 2, 2>(yaml, "fees", Store(table));
  ...
  TEST_CASE("2d_array_invalid_row_size") {
    ...
    auto table = std::array<std::array<Quantity, 2>, 2>();
    REQUIRE_THROWS_AS((parse_fee_table<Quantity, 2, 2>(yaml, Store(table))),
      std::runtime_error);
  }

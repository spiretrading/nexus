...
namespace {
  auto make_security(std::string symbol, Venue venue) {
    return Security(std::move(symbol), venue);
  }
  ...
  auto make_order(Security security, Money price, Quantity quantity = 100) {
    auto fields = make_order_fields(security, price, quantity);
    auto info = OrderInfo(
...
TEST_SUITE("consolidated_tmx_fee_table") {
  TEST_CASE("zero_quantity") {
    ...
    auto order = make_order(security, Money::ONE, 0);
  ...
  TEST_CASE("etf_fee_routing") {
    auto table = make_fee_table();
    auto state = ConsolidatedTmxFeeTable::State();
    auto security = make_security("ETF", TSX);
    auto order = make_order(security, Money::ONE, 100);
  ...
  TEST_CASE("nex_listed_fee_routing") {
    auto table = make_fee_table();
    auto state = ConsolidatedTmxFeeTable::State();
    auto security = make_security("NEX", TSXV);
    auto order = make_order(security, Money::ONE, 100);

...
namespace Nexus {
  ...
  class ComplianceRuleSet {
    ...
    private:
      ...
      Beam::SynchronizedUnorderedMap<
        Beam::ServiceLocator::DirectoryEntry, std::shared_ptr<Entry>> m_entries;
  ...
  std::shared_ptr<typename ComplianceRuleSet<C, S>::Entry>
      ComplianceRuleSet<C, S>::load(
        const Beam::ServiceLocator::DirectoryEntry& directory_entry) {
    auto entry = m_entries.GetOrInsert(
    ...
    entry->m_initializer.Call([&] {
      ...
      auto queue = std::deque<Beam::ServiceLocator::DirectoryEntry>();
      queue.push_back(directory_entry);
      while(!queue.empty()) {
        auto current = std::move(queue.front());
        queue.pop_front();
        auto parents = m_service_locator_client->LoadParents(current);
      ...
      m_compliance_client->monitor_compliance_rule_entries(
        directory_entry, m_tasks.GetSlot<ComplianceRuleEntry>(

...
namespace Nexus {
  ...
  concept IsHistoricalDataStore = Beam::IsConnection<T> && requires(T& store) {
    ...
    { store.load_order_imbalances(std::declval<const VenueQuery&>()) } ->
    ...
    private:
      struct VirtualHistoricalDataStore {
        ...
        virtual std::vector<TickerInfo> load_ticker_info(
          const TickerInfoQuery& query) = 0;
        virtual void store(const TickerInfo& info) = 0;
        virtual std::vector<SequencedOrderImbalance> load_order_imbalances(
          const VenueQuery& query) = 0;
        ...
        virtual void store(const SequencedTickerBboQuote& quote) = 0;
        virtual void store(
          const std::vector<SequencedTickerBboQuote>& quotes) = 0;
      ...
      struct WrappedHistoricalDataStore final : VirtualHistoricalDataStore {
        ...
        std::vector<SequencedOrderImbalance> load_order_imbalances(
          const VenueQuery& query) override;
  ...
  auto load(D& data_store, const VenueQuery& query) {
  ...
  inline std::vector<SequencedOrderImbalance>
      HistoricalDataStore::load_order_imbalances(const VenueQuery& query) {
  ...
  inline std::vector<SequencedBookQuote> HistoricalDataStore::load_book_quotes(
      const TickerQuery& query) {

...
namespace Nexus::MarketDataService {
  ...
  class SqlHistoricalDataStore {
    public:
       ...
       */
      explicit SqlHistoricalDataStore(VenueDatabase venues,
  ...
  std::vector<SecurityInfo> SqlHistoricalDataStore<C>::load_security_info(
    ...
    auto region_filter = Viper::literal(query.GetIndex().is_global());
    for(auto country : query.GetIndex().get_countries()) {
      for(auto entry : m_venues.get_entries()) {
        if(entry.m_country_code == country) {
          region_filter = region_filter || Viper::sym("venue") ==
            Viper::literal(std::string(entry.m_venue.get_code().GetData()));
        }
      }
    }
    for(auto venue : query.GetIndex().get_venues()) {
      region_filter = region_filter || Viper::sym("venue") ==
        Viper::literal(std::string(venue.get_code().GetData()));
    }
    for(auto& security : query.GetIndex().get_securities()) {

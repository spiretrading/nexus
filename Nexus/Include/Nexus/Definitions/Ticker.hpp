...
namespace Nexus {
  ...
  inline std::size_t hash_value(const Ticker& ticker) {
    auto seed = std::size_t(0);
    boost::hash_combine(seed, ticker.get_symbol());
    boost::hash_combine(seed, ticker.get_venue());
    return seed;
  }
...
namespace Nexus {
  ...
  /**
   * Parses a set of tickers from a YAML config.
  ...
  /**
   * Parses a set of tickers from a YAML config using the default venues.
   * @param config The config to parse.
   * @return The set of parsed tickers.
   */

#ifndef NEXUS_EXCHANGERATE_HPP
#define NEXUS_EXCHANGERATE_HPP
...
namespace Nexus {
  ...
  /*! \struct ExchangeRate
      \brief Represents a CurrencyPair's exchange rate.
  */
  struct ExchangeRate {
    ...
    /*!
      ...
      \param rate The conversion rate from the base currency to the counter
             currency.
    */
    ExchangeRate(const CurrencyPair& pair, const boost::rational<int>& rate);
  };
  ...
  */
  ...
  inline ExchangeRate::ExchangeRate(const CurrencyPair& pair,
      const boost::rational<int>& rate)
      : m_pair(pair),
        m_rate(rate) {}
}

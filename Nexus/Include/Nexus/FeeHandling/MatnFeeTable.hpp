#ifndef NEXUS_MATNFEETABLE_HPP
#define NEXUS_MATNFEETABLE_HPP
#include <array>
...
namespace Nexus {
  ...
  /*! \struct MatnFeeTable
      \brief Stores the table of fees used by Match NOW.
   */
  struct MatnFeeTable {
    ...
    /*! \enum GeneralIndex
        \brief Enumerates the indices used in the general fee table.
     */
    enum class GeneralIndex : int {
    ...
    static const std::size_t GENERAL_INDEX_COUNT = 2;

    /*! \enum Category
        \brief Enumerates the categories for alternative trades.
     */
    ...
    if(classification == MatnFeeTable::Classification::DEFAULT &&
      ...
      auto priceClass = [&] {
        if(executionReport.m_lastPrice < Money::ONE) {
          return MatnFeeTable::PriceClass::SUB_DOLLAR;
        } else if(executionReport.m_lastPrice < 5 * Money::ONE) {
          return MatnFeeTable::PriceClass::SUB_FIVE_DOLLAR;
        } else {

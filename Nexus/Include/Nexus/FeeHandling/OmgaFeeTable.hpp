#ifndef NEXUS_OMGAFEETABLE_HPP
#define NEXUS_OMGAFEETABLE_HPP
#include <array>
...
namespace Nexus {
  ...
  /*! \struct OmgaFeeTable
      \brief Stores the table of fees used by Omega on TSX.
   */
  struct OmgaFeeTable {
  ...
  inline Money CalculateFee(const OmgaFeeTable& feeTable, bool isEtf,
    ...
    auto priceClass = [&] {
      if(executionReport.m_lastPrice < Money::ONE) {
        return OmgaFeeTable::PriceClass::SUB_DOLLAR;
      } else {

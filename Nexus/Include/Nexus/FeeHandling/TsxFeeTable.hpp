...
namespace Nexus {
  ...
  inline TsxFeeTable parse_tsx_fee_table(const YAML::Node& config) {
    ...
    parse_fee_table(config, "continuous_fee_table",
      Beam::Store(table.m_continuous_fee_table));
    parse_fee_table(
      config, "auction_fee_table", Beam::Store(table.m_auction_fee_table));
    parse_fee_table(
      config, "odd_lot_list", Beam::Store(table.m_odd_lot_fee_list));
    return table;
  }
   ...
   * @param table The TsxFeeTable used to lookup the fee.
   * @param auction_index The AuctionIndex to lookup.
   * @param auction_type The AuctionType to lookup.
   * @return The fee corresponding to the specified <i>auction_index</i> and
   *         <i>auction_type</i>.
   */
  inline Money lookup_auction_fee(const TsxFeeTable& table,
      TsxFeeTable::AuctionIndex auction_index,
      TsxFeeTable::AuctionType auction_type) {
    return table.m_auction_fee_table[static_cast<int>(auction_type)][
      static_cast<int>(auction_index)];
  }
   ...
   */
  inline Money lookup_odd_lot_fee(const TsxFeeTable& table,
      TsxFeeTable::PriceClass price_class) {

...
namespace Nexus {
  ...
  /** Consolidates all TMX related venue fees together. */
  ...
  /**
   * Calculates the fee on a trade executed on a TMX venue.
      ...
      if(last_market == DefaultVenues::XATS) {
        ...
        auto ticker_class = [&] {
          if(table.m_etfs.contains(order.get_info().m_fields.m_ticker)) {
            ...
            return CxdFeeTable::TickerClass::DEFAULT;
          }
        }();

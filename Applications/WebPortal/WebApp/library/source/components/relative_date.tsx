  ...
  const content = (() => {
    switch(relativeDay) {
      case RelativeDay.TODAY:
        return props.datetime.toLocaleTimeString('en-US', {
          hour: '2-digit', minute: '2-digit', hour12: false
        });
      case RelativeDay.YESTERDAY:
        return 'yesterday';
      case RelativeDay.OTHER_DAY:
        return props.datetime.toLocaleDateString('en-US', {
          month: 'short', day: 'numeric', year: 'numeric'
        });
    }
  })();
  return (
    <time className={css(STYLES.time)} dateTime={props.datetime.toISOString()}>
      {content}
    </time>);
}

function getRelativeDay(datetime: Date, today: Date): RelativeDay {
  const dtYear = datetime.getFullYear();
  const dtMonth = datetime.getMonth();
  const dtDate = datetime.getDate();

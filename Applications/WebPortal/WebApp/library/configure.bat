...
IF NOT EXIST build.bat (
  ECHO @ECHO OFF > build.bat || EXIT /B 1
  ECHO CALL "%~dp0build.bat" %%* >> build.bat || EXIT /B 1
)
IF NOT EXIST configure.bat (
  ECHO @ECHO OFF > configure.bat || EXIT /B 1
  ECHO CALL "%~dp0configure.bat" %%* >> configure.bat || EXIT /B 1
)
EXIT /B 0

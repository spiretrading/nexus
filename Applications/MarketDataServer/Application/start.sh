...
ports=()
for server in registry_server feed_server; do
  interface=$(yq -r ".$server.interface // \"\"" "$CONFIG_FILE") || exit 1
  port=${interface##*:}
  if [[ "$interface" != *:* || ! "$port" =~ ^[0-9]{1,5}$ ]]; then
    echo "Error: $server must specify an interface with a TCP port." >&2
    exit 1
  fi
  port=$((10#$port))
  if((port < 1 || port > 65535)); then
    echo "Error: Invalid TCP port for $server in $CONFIG_FILE." >&2
    exit 1
  fi
  ports+=("$port")
done

...
namespace Details {
  ...
  /**
   * A ValueModel that transforms the current value of a source model via a
   * callable.
   * @param <T> The type of value to model.
   * @param <U> The type of the source's current value.
    ...
    public:
      ...
      TransformValueModel(
          ...
          : m_source(std::move(source)),
            ...
            m_model(m_f(m_source->get_current())) {
        m_source->connect_current_signal(
          std::bind_front(&TransformValueModel::on_current, this));
      }
  ...
  const typename TransformValueModel<T, U, F, G>::Type&
      TransformValueModel<T, U, F, G>::get_current() const {
  ...
  QValidator::State TransformValueModel<T, U, F, G>::set_current(
      const Type& value) {
    try {
      return m_source->set_current(m_g(m_source->get_current(), value));

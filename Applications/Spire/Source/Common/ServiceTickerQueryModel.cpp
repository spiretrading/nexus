...
ServiceTickerQueryModel::ServiceTickerQueryModel(
  MarketDataClient market_data_client)
  : m_market_data_client(std::move(market_data_client)) {}

optional<TickerInfo> ServiceTickerQueryModel::parse(const QString& query) {
...
QtPromise<std::vector<TickerInfo>>
    ServiceTickerQueryModel::submit(const QString& query) {
  if(query.isEmpty()) {
    return {};
  }

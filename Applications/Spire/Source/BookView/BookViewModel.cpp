...
QVariant BookViewModel::data(const QModelIndex& index, int role) const {
  ...
  if(role == Qt::TextAlignmentRole) {
    ...
    auto highlight = m_properties.GetVenueHighlight(entry.m_quote.m_venue);
...
bool BookViewModel::TestHighlight(
    const BookViewProperties::VenueHighlight& highlight,
    const BookQuote& quote) const {
  if(highlight.m_highlightAllLevels) {
    return true;
  }
  auto topLevelIterator = m_topLevels.find(quote.m_venue);
...
void BookViewModel::HighlightQuote(const BookQuote& quote) {
  auto topQuoteIterator = m_topLevels.find(quote.m_venue);
  if(topQuoteIterator == m_topLevels.end()) {
    if(quote.m_quote.m_size != 0) {
      m_topLevels.insert(std::pair(quote.m_venue, quote));
    }
  } else {
    ...
    if(quote.m_quote.m_size == 0 && quote.m_mpid == topQuote.m_mpid &&
      ...
      while(i != m_bookQuotes.rend() &&
        ...
        auto& entry = **i;
        if(newTopQuoteIndex == -1 && entry.m_quote.m_venue == quote.m_venue &&
      ...
      if(newTopQuoteIndex != -1) {
        ...
        m_topLevels.erase(quote.m_venue);
      }
      if(previousTopQuoteIndex != -1) {
        ...
        (-get_direction(m_side) * quote.m_quote.m_price <
        -get_direction(m_side) * topQuote.m_quote.m_price ||
        quote.m_quote.m_price == topQuote.m_quote.m_price &&
        quote.m_is_primary_mpid && !topQuote.m_is_primary_mpid)) {
...
void BookViewModel::OnBookQuote(const BookQuote& quote) {
  ...
  auto direction = get_direction(m_side);
  auto lowerBound = [&] {
...
void BookViewModel::OnExecutionReport(const std::shared_ptr<Order>& order,
  ...
  key.m_destination = order->get_info().m_fields.m_destination;
  if(executionReport.m_status == OrderStatus::PENDING_NEW) {
    m_remainingOrderQuantities.insert(
      std::pair(order, order->get_info().m_fields.m_quantity));
    auto& quantity = m_orderQuantities[key];
    quantity += order->get_info().m_fields.m_quantity;
    if(m_properties.GetOrderHighlight() != BookViewProperties::HIDE_ORDERS) {
      auto bookQuote = BookQuote("@" + key.m_destination, false, Venue(),
        Quote(key.m_price, quantity, m_side), executionReport.m_timestamp);
      OnBookQuote(bookQuote);
    }
  } else if(executionReport.m_last_quantity != 0 ||
      is_terminal(executionReport.m_status)) {
    ...
    if(is_terminal(executionReport.m_status)) {
      ...
      quantity -= executionReport.m_last_quantity;
    }
    if(m_properties.GetOrderHighlight() != BookViewProperties::HIDE_ORDERS) {
      auto bookQuote = BookQuote("@" + key.m_destination, false, Venue(),
        Quote(key.m_price, quantity, m_side), executionReport.m_timestamp);
  ...
  m_remainingOrderQuantities[order] -= executionReport.m_last_quantity;
}

...
struct BookViewModel::PreviewOrderTracker {
  BookViewModel* m_model;
  ListIndexTracker m_preview_order_index;
  Side m_preview_order_side;
  scoped_connection m_operation_connection;
  scoped_connection m_update_connection;

  explicit PreviewOrderTracker(BookViewModel& model)
    : m_model(&model),
      m_preview_order_index(-1),
      m_preview_order_side(Side::NONE),
      m_update_connection(m_model->get_preview_order()->connect_update_signal(
        std::bind_front(&PreviewOrderTracker::on_update, this))) {}
    ...
    if(order) {
      ...
      if(m_preview_order_index.get_index() != -1) {
        ...
        m_operation_connection = quotes->connect_operation_signal(
          std::bind_front(&PreviewOrderTracker::on_operation, this));
...
BookViewModel::BookViewModel()
  : m_bids(std::make_shared<ArrayListModel<BookQuote>>()),
    ...
    m_preview_order_tracker(std::make_shared<PreviewOrderTracker>(*this)) {}

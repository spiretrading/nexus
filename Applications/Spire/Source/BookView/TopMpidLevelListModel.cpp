  ...
  visit(operation,
    [&] (const BookQuoteListModel::AddOperation& operation) {
      ...
      if(quote.m_isPrimaryMpid) {
        auto top_index = find_level(m_top_mpids, quote.m_market);
        if(top_index != -1) {
          auto& top_mpid = m_top_mpids.get(top_index);
          if(top_mpid.m_level >= operation.m_index) {
            m_top_mpids.set(
              top_index, TopMpidLevel(quote.m_market, operation.m_index));
          }
        } else {
          m_top_mpids.push(TopMpidLevel(quote.m_market, operation.m_index));
        }
      }
      for(auto i = 0; i != m_top_mpids.get_size(); ++i) {
        ...
        if(top_mpid.m_level >= operation.m_index &&
          ...
          m_top_mpids.set(
            i, TopMpidLevel(top_mpid.m_market, top_mpid.m_level + 1));
        }
      }
    },
    ...
    [&] (const BookQuoteListModel::RemoveOperation& operation) {
      if(m_removed_quote.m_isPrimaryMpid) {
        auto top_index = find_level(m_top_mpids, m_removed_quote.m_market);
        auto& top_mpid = m_top_mpids.get(top_index);
        if(top_mpid.m_level >= operation.m_index) {
          for(auto i = operation.m_index; i != m_quotes->get_size(); ++i) {
            if(m_quotes->get(i).m_isPrimaryMpid &&
              ...
              m_top_mpids.set(top_index, TopMpidLevel(top_mpid.m_market, i));
              break;
            }
          }
        }

...
MarketDepth::MarketDepth(std::shared_ptr<BookViewModel> model,
  ...
  m_bid_position_connection = bid_vertical_scroll_bar->connect_position_signal(
    [=] (auto position) {
      ...
      ask_vertical_scroll_bar->set_position(position);
    });
  m_ask_position_connection = ask_vertical_scroll_bar->connect_position_signal(
    [=] (auto position) {
      ...
      bid_vertical_scroll_bar->set_position(position);
    });
  m_selected_quote->connect_update_signal(
    std::bind_front(&MarketDepth::on_current_quote, this));
}
...
void MarketDepth::focusInEvent(QFocusEvent* event) {
  if(event->reason() == Qt::ActiveWindowFocusReason) {
    if(!m_selected_quote->get()) {
      if(m_bid_table_view->get_table()->get_row_size() > 0) {
...
void MarketDepth::on_current_quote(const optional<BookQuote>& current) {
  if(!current) {
    setFocus();
  }
}

...
void TickerInfoModel::Search(const std::string& prefix) {
  ...
  m_queryPromise = QtPromise([=, userProfile = m_userProfile] {
    return userProfile->GetClients().
      get_market_data_client().load_ticker_info_from_prefix(uppercasePrefix);
...
QVariant TickerInfoModel::data(const QModelIndex& index, int role) const {
  ...
  if(role == Qt::DisplayRole) {
    if(index.column() == TICKER_COLUMN) {
      return QString::fromStdString(to_string(item.m_ticker));
...
QVariant TickerInfoModel::headerData(int section, Qt::Orientation orientation,
    int role) const {
  if(role == Qt::DisplayRole) {

...
namespace {
  ...
  auto setup_decimal_box_profile(UiProfile& profile) {
    ...
    current.connect_changed_signal([=] (auto value) {
      if(box->get_current()->get_current() != value) {
  ...
  auto setup_checkable_profile(UiProfile& profile, CheckBox* check_box) {
    ...
    checked.connect_changed_signal([=] (auto value) {
      if(check_box->get_current()->get_current() != value) {
        check_box->get_current()->set_current(value);
      }
    });
...
UiProfile Spire::make_calendar_date_picker_profile() {
  ...
  auto profile = UiProfile(QString::fromUtf8("CalendarDatePicker"), properties,
    [] (auto& profile) {
      ...
      calendar->get_current()->connect_current_signal(profile.make_event_slot<
    ...
    [] (auto& profile) {
      ...
      current.connect_changed_signal([=] (const auto& value) {
        if(value == QString::fromUtf8("null")) {
          if(decimal_box->get_current()->get_current()) {
            decimal_box->get_current()->set_current(none);
          }
        } else if(auto decimal = parse_decimal(value)) {
          if(decimal_box->get_current()->get_current() != *decimal) {
            decimal_box->get_current()->set_current(*decimal);
          }
        }
      });
      auto current_slot =
    ...
    [] (auto& profile) {
      ...
      current.connect_changed_signal([=] (auto value) {
        if(auto current_value = parse_duration(value)) {
          if(duration_box->get_current()->get_current() != *current_value) {
            duration_box->get_current()->set_current(*current_value);
          }
        }
      });
      auto& read_only = get<bool>("read_only", profile.get_properties());
      ...
      auto widget = [&] () -> QWidget* {
        ...
        if(value == 0) {
          ...
          QObject::connect(timer, &QTimer::timeout, [=] {
            if(auto& current = list_view->get_current()->get_current()) {
              list_view->get_current()->set_current(
  ...
  auto profile = UiProfile(QString::fromUtf8("KeyTag"), properties,
    [] (auto& profile) {
      ...
      key.connect_changed_signal([=] (auto key_text) {
        ...
        key_tag->get_current()->set_current(key);
      });
      return key_tag;
    });
  return profile;
  ...
  auto profile = UiProfile(QString::fromUtf8("NavigationView"), properties,
    [=] (auto& profile) {
      ...
      current.connect_changed_signal([=] (auto index) {
        if(index >= 0 && index < navigation_view->get_count()) {
          navigation_view->get_current()->set_current(index);
        }
      });
...
UiProfile Spire::make_text_area_box_profile() {
  ...
  auto profile = UiProfile(QString::fromUtf8("TextAreaBox"), properties,
    [] (auto& profile) {
      ...
      auto& current = get<QString>("current", profile.get_properties());
      current.connect_changed_signal([=] (const auto& value) {
        if(text_area_box->get_current()->get_current() != value) {
          text_area_box->get_current()->set_current(value);
        }
      });
...
UiProfile Spire::make_text_box_profile() {
  ...
  auto profile = UiProfile(QString::fromUtf8("TextBox"), properties,
    [] (auto& profile) {
      ...
      auto& current = get<QString>("current", profile.get_properties());
      current.connect_changed_signal([=] (const auto& current) {
        if(text_box->get_current()->get_current() != current) {
          text_box->get_current()->set_current(current);
        }
      });
      auto& placeholder = get<QString>("placeholder", profile.get_properties());
      placeholder.connect_changed_signal([=] (const auto& text) {
    ...
    [] (auto& profile) {
      ...
      current.connect_changed_signal([=] (auto value) {
        if(auto current_value = parse_time(value)) {
          if(time_box->get_current()->get_current() != *current_value) {
            time_box->get_current()->set_current(*current_value);
          }
        }
      });
      auto& read_only = get<bool>("read_only", profile.get_properties());

...
MoneySpinBox::MoneySpinBox(RefType<UserProfile> userProfile, QWidget* parent,
    Qt::WindowFlags flags)
    : MoneySpinBox{parent, flags} {
  Initialize(Ref(userProfile));
}

void MoneySpinBox::Initialize(RefType<UserProfile> userProfile) {
...
void MoneySpinBox::SetValue(Money value) {
  m_spinBox->setValue(
    value.GetRepresentation() / static_cast<double>(Money::MULTIPLIER));
...
void MoneySpinBox::AdjustIncrement(KeyModifiers modifier) {
  ...
  if(increment != m_spinBox->singleStep()) {
    m_spinBox->setSingleStep(increment);
  }
}

...
NewBlotterForm::NewBlotterForm(DirectoryEntry account,
    std::shared_ptr<AccountQueryModel> accounts, bool is_account_selectable,
    QWidget& parent)
    : QWidget(&parent),
      m_account(std::move(account)),
      m_account_box(nullptr) {
  auto heading_label = make_label(tr("New Blotter"));
  ...
  m_name_connection = m_name_model->connect_update_signal(
    std::bind_front(&NewBlotterForm::on_name_current, this));
  body_layout->addWidget(name_label);
  body_layout->addWidget(m_name_input);
...
bool NewBlotterForm::eventFilter(QObject* watched, QEvent* event) {
  ...
  } else if(watched == m_name_input) {
    if(event->type() == QEvent::ChildAdded) {
      auto& child = *static_cast<QChildEvent&>(*event).child();
      if(child.isWidgetType()) {
        child.installEventFilter(this);
        m_name_input->removeEventFilter(this);
      }
    }

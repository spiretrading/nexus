...
TickerListItem::TickerListItem(TickerInfo ticker_info, QWidget* parent)
    : QWidget(parent),
      m_ticker_info(std::move(ticker_info)) {
  auto value_label =
    make_label(QString::fromStdString(to_string(m_ticker_info.m_ticker)));
  ...
  auto name_label = make_label(QString::fromStdString(m_ticker_info.m_name));
...
const TickerInfo& TickerListItem::get_ticker() const {
  return m_ticker_info;
}

...
namespace {
  ...
  auto make_header_model() {
    ...
    push(QObject::tr("Scope"), QObject::tr("Scope"));
  ...
  struct UniqueTaskKeyTableModel : TableModel {
    ...
    static const auto SCOPE_INDEX = static_cast<int>(OrderTaskColumns::SCOPE);
    std::shared_ptr<TableModel> m_source;
    TableModelTransactionLog m_transaction;
    std::unordered_set<std::pair<Scope, QKeySequence>, ScopeKeyHash>
      m_scope_keys;
    ...
    explicit UniqueTaskKeyTableModel(std::shared_ptr<TableModel> source)
      ...
      for(auto row = 0; row < get_row_size(); ++row) {
        if(auto& key = get<QKeySequence>(row, KEY_INDEX); !key.isEmpty()) {
          m_scope_keys.insert({get<Scope>(row, SCOPE_INDEX), key});
        }
      }
    }
    ...
    QValidator::State set(int row, int column, const std::any& value) override {
      auto find_conflicting_row =
        [&] (const Scope& scope, const QKeySequence& key) {
          if(key.isEmpty() || m_scope_keys.insert({scope, key}).second) {
            return -1;
          }
          for(auto i = 0; i < get_row_size(); ++i) {
            if(i != row && get<Scope>(i, SCOPE_INDEX) == scope &&
      ...
      m_transaction.transact([&] {
        if(column == SCOPE_INDEX) {
          auto& key = get<QKeySequence>(row, KEY_INDEX);
          m_scope_keys.erase({get<Scope>(row, SCOPE_INDEX), key});
          auto row =
            find_conflicting_row(std::any_cast<const Scope&>(value), key);
          if(row != -1) {
            result = m_source->set(row, KEY_INDEX, QKeySequence());
          }
        } else if(column == KEY_INDEX) {
          auto& scope = get<Scope>(row, SCOPE_INDEX);
          m_scope_keys.erase({scope, get<QKeySequence>(row, KEY_INDEX)});
          auto row = find_conflicting_row(
            scope, std::any_cast<const QKeySequence&>(value));
          if(row != -1) {
            result = m_source->set(row, KEY_INDEX, QKeySequence());
          }
        }
    ...
    QValidator::State remove(int row) override {
      m_scope_keys.erase(
        {get<Scope>(row, SCOPE_INDEX), get<QKeySequence>(row, KEY_INDEX)});
  ...
  struct DestinationState : ItemState {
    std::shared_ptr<ProxyValueModel<Scope>> m_scope;

    DestinationState(std::shared_ptr<void> proxy,
      std::shared_ptr<ProxyValueModel<Scope>> scope)
  ...
  struct TaskKeysTableViewItemBuilder {
    ...
    EditableBox* mount(
      ...
      auto [input_box, proxy] =
        [&] () -> std::tuple<EditableBox*, std::shared_ptr<ItemState>> {
          if(column_id == OrderTaskColumns::NAME) {
          ...
          } else if(column_id == OrderTaskColumns::SCOPE) {
            auto current = make_proxy.operator ()<Scope>();
            auto scope_box = new ScopeBox(m_scopes, current);
            auto breakout_input_box =
              new BreakoutInputBox(*new AnyInputBox(*scope_box));
            auto editable_box =
            ...
            auto scope = make_proxy_value_model(
              make_table_value_model<Scope>(table, row,
                static_cast<int>(OrderTaskColumns::SCOPE)));
            auto destinations = make_scope_filtered_destination_list(scope);
            auto current = make_proxy.operator ()<Destination>();
            return {new EditableBox(
              *make_destination_box(current, std::move(destinations))),
              std::make_shared<DestinationState>(current, scope)};
            ...
            auto scope = make_proxy_value_model(make_table_value_model<Scope>(
              table, row, static_cast<int>(OrderTaskColumns::SCOPE)));
            auto current = make_proxy.operator ()<std::vector<AdditionalTag>>();
            return {new EditableBox(*new AdditionalTagsBox(
              current, m_additional_tags, destination, scope)),
    ...
    void reset(QWidget& widget,
      ...
      if(column_id == OrderTaskColumns::NAME) {
        ...
        state.m_scope->set_source(make_table_value_model<Scope>(
          table, row, static_cast<int>(OrderTaskColumns::SCOPE)));
...
TableView* Spire::make_task_keys_table_view(
  ...
  auto builder = EditableTableViewBuilder(
    ...
    set_item_builder(RecycledTableViewItemBuilder(
      TaskKeysTableViewItemBuilder(scopes, additional_tags))).

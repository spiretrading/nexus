...
TEST_SUITE("IsTopMpidModel") {
  TEST_CASE("constructor_empty") {
    ...
    auto entry = make_local_value_model<BookEntry>(TEST_MPID);
    auto price = make_local_value_model(2 * Money::ONE);
  ...
  TEST_CASE("constructor_missing_mpid") {
    ...
    auto top_mpid_prices = std::make_shared<TopMpidPriceListModel>(quotes);
    auto entry = make_local_value_model<BookEntry>(TEST_MPID);
    auto price = make_local_value_model(Money::ONE);
    auto is_top = IsTopMpidModel(top_mpid_prices, entry, price);
    REQUIRE(!is_top.get());
    SUBCASE("update_mpid") {
  ...
  TEST_CASE("constructor_missing_origin") {
    ...
    SUBCASE("user_order") {
      ...
      auto entry = make_local_value_model<BookEntry>(missing_mpid);
      auto is_top = IsTopMpidModel(top_mpid_prices, entry, price);
      REQUIRE(!is_top.get());
    }
    SUBCASE("preview") {
  ...
  TEST_CASE("change_from_a_top_quote_to_a_user_order") {
    ...
    REQUIRE(is_top.get());
    entry->set(
      BookViewModel::UserOrder("TSX", 2 * Money::ONE, 100, OrderStatus::NEW));
    REQUIRE(!is_top.get());
  }
}

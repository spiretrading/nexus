...
# stop_server.sh
# 
# Use this script to stop a AdministrationServer.
#
# Usage: ./stop_server.sh    Stops the AdministrationServer.
...
if [ "$check_run" = "0" ]; then
  ...
  echo "${red}[ERROR]${reset} Could not terminate ${yellow}AdministrationServer${reset}."
  echo "        ${yellow}AdministrationServer${reset} is not active."
else
  # AdministrationServer is already running.
  pid=$(ps -ef | grep -i "AdministrationServer" | grep -v "grep" | grep -v "bash" | awk '{ print $2 }')
  kill -s INT $pid
  echo "${yellow}AdministrationServer${reset} terminated."

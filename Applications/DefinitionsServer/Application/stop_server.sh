...
# stop_server.sh
# 
# Use this script to stop a DefinitionsServer.
#
# Usage: ./stop_server.sh    Stops the DefinitionsServer.
...
if [ "$check_run" = "0" ]; then
  ...
  echo "${red}[ERROR]${reset} Could not terminate ${yellow}DefinitionsServer${reset}."
  echo "        ${yellow}DefinitionsServer${reset} is not active."
else
  # DefinitionsServer is already running.
  pid=$(ps -ef | grep -i "DefinitionsServer" | grep -v "grep" | grep -v "bash" | awk '{ print $2 }')
  kill -s INT $pid
  echo "${yellow}DefinitionsServer${reset} terminated."

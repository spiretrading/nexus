...
# stop_server.sh
# 
# Use this script to stop a ComplianceServer.
#
# Usage: ./stop_server.sh    Stops the ComplianceServer.
...
if [ "$check_run" = "0" ]; then
  ...
  echo "${red}[ERROR]${reset} Could not terminate ${yellow}ComplianceServer${reset}."
  echo "        ${yellow}ComplianceServer${reset} is not active."
else
  # ComplianceServer is already running.
  pid=$(ps -ef | grep -i "ComplianceServer" | grep -v "grep" | grep -v "bash" | awk '{ print $2 }')
  kill -s INT $pid
  echo "${yellow}ComplianceServer${reset} terminated."

...
const ScopeMap<InteractionsProperties>& InteractionsWidget::GetProperties() {
  Store();
  return m_properties;
}
...
void InteractionsWidget::AddScope(ScopeEntry scope) {
  ...
  m_ui->m_scopeComboBox->addItem(QString::fromStdString(
    scope.m_scope.get_name()));
...
void InteractionsWidget::OnActivateScopeClicked() {
  ...
  if(!scope.m_isActive) {
    ...
    m_properties.set(scope.m_scope,

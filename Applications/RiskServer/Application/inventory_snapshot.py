...
def load_order_records(account, data_store):
  query = beam.Query()
  query.index = account
  query.range = beam.Range.TOTAL
  query.snapshot_limit = beam.SnapshotLimit.UNLIMITED
  return data_store.load_order_records(query)
...
def recalculate_snapshot(account, data_store):
  ...
  return model.make_snapshot()

def update_snapshot(
    account, source, order_execution_data_store, risk_data_store):
  snapshot = recalculate_snapshot(account, source)
...
def main():
  ...
  if args.output:
    ...
    for account in accounts:
      ...
      restore_snapshot(account, risk_source, risk_destination)

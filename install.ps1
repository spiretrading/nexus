...
If((Get-Command "grep.exe" -ErrorAction SilentlyContinue) -eq $null) {
  ...
  Invoke-WebRequest -UseBasicParsing -Uri https://eternallybored.org/misc/wget/1.21.1/64/wget.exe -UserAgent [Microsoft.PowerShell.Commands.PSUserAgent]::FireFox -OutFile wget.exe
...
If((Get-Command "git.exe" -ErrorAction SilentlyContinue) -eq $null) {
  ...
  Invoke-WebRequest -UseBasicParsing -Uri https://github.com/git-for-windows/git/releases/download/v2.32.0.windows.1/Git-2.32.0-64-bit.exe -OutFile Git-2.32.0-64-bit.exe
  .\Git-2.32.0-64-bit.exe /SILENT /LOADINF="git_config.inf" | Out-Null
  rm .\Git-2.32.0-64-bit.exe -Force
  rm .\git_config.inf -Force
}
If((Get-Command "cmake.exe" -ErrorAction SilentlyContinue) -eq $null) {
  Invoke-WebRequest -UseBasicParsing -Uri https://github.com/Kitware/CMake/releases/download/v3.20.4/cmake-3.20.4-windows-x86_64.msi -OutFile cmake-3.20.4-windows-x86_64.msi
  msiexec /quiet /i cmake-3.20.4-windows-x86_64.msi ADD_CMAKE_TO_PATH=User | Out-Null
  rm .\cmake-3.20.4-windows-x86_64.msi -Force
}
If((Get-Command "node.exe" -ErrorAction SilentlyContinue) -eq $null) {
  Invoke-WebRequest -UseBasicParsing -Uri https://nodejs.org/dist/v16.3.0/node-v16.3.0-x64.msi -OutFile node-v16.3.0-x64.msi
  msiexec /quiet /i node-v16.3.0-x64.msi | Out-Null
  rm .\node-v16.3.0-x64.msi
}

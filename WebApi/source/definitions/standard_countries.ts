...
export function buildCountryDatabase(): CountryDatabase {
  const database = new CountryDatabase();
  database.add(
    new CountryDatabase.Entry(new CountryCode(36), 'Australia', 'AU', 'AUS'));
  database.add(
    new CountryDatabase.Entry(new CountryCode(76), 'Brazil', 'BR', 'BRA'));
  database.add(
    new CountryDatabase.Entry(new CountryCode(124), 'Canada', 'CA', 'CAN'));
  database.add(
    new CountryDatabase.Entry(new CountryCode(156), 'China', 'CN', 'CHN'));
  database.add(new CountryDatabase.Entry(
    new CountryCode(840), 'United States', 'US', 'USA'));
  return database;
}

export const countryDatabase = buildCountryDatabase();

...
describe('CurrencyDatabase', () => {
  it('none_entry', () => {
    ...
    assert.strictEqual(none.name, 'Unknown');
  ...
  it('currency_lookup', () => {
    ...
    database.add(
      new CurrencyDatabase.Entry(new Currency(10), 'AAA', 'Alpha', 'A$'));
    database.add(
      new CurrencyDatabase.Entry(new Currency(5), 'BBB', 'Beta', 'B$'));
    assert.strictEqual(database.fromCurrency(new Currency(10)).sign, 'A$');
    assert.strictEqual(database.fromCurrency(new Currency(5)).sign, 'B$');
    assert.ok(
  ...
  it('code_lookup', () => {
    const database = new CurrencyDatabase();
    database.add(
      new CurrencyDatabase.Entry(new Currency(20), 'CCC', 'Charlie', 'C$'));
    assert.ok(database.fromCode('CCC').currency.equals(new Currency(20)));
    assert.ok(database.fromCode('XXX').currency.equals(Currency.NONE));
  });

  it('remove', () => {
    const database = new CurrencyDatabase();
    database.add(
      new CurrencyDatabase.Entry(new Currency(30), 'DDD', 'Delta', 'D$'));
    assert.ok(database.fromCurrency(new Currency(30)).currency.equals(
      new Currency(30)));
  ...
  it('add_duplicate_ignored', () => {
    ...
    database.add(
      new CurrencyDatabase.Entry(new Currency(10), 'AAA', 'Alpha', 'A$'));
    database.add(
      new CurrencyDatabase.Entry(new Currency(20), 'AAA', 'Alpha2', 'X$'));
    assert.strictEqual(database.fromCode('AAA').sign, 'A$');
  });

  it('entry_equals', () => {
    const a =
      new CurrencyDatabase.Entry(new Currency(840), 'USD', 'US Dollar', '$');
    const b =
      new CurrencyDatabase.Entry(new Currency(840), 'USD', 'US Dollar', '$');
    assert.ok(a.equals(b));
  });
  ...
  it('entry_not_equal', () => {
    const a =
      new CurrencyDatabase.Entry(new Currency(840), 'USD', 'US Dollar', '$');
    const b = new CurrencyDatabase.Entry(
      new Currency(124), 'CAD', 'Canadian Dollar', 'C$');
    assert.ok(!a.equals(b));
  });
  ...
  it('round_trip_json', () => {
    ...
    assert.strictEqual(restored.fromCode('USD').currency.code, 840);
    assert.strictEqual(restored.fromCode('CAD').currency.code, 124);
  });

  it('iterable', () => {
    const database = new CurrencyDatabase();
    database.add(
      new CurrencyDatabase.Entry(new Currency(1), 'AAA', 'Alpha', 'A'));
    database.add(
      new CurrencyDatabase.Entry(new Currency(2), 'BBB', 'Beta', 'B'));
